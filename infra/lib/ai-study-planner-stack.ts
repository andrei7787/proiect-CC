...
const apiEntry = (relative: string) =>
	path.join(
...
export class AiStudyPlannerStack extends Stack {
	constructor(scope: Construct, id: string, props?: StackProps) {
		...
		const materialDlq = new Queue(this, "MaterialDlq", {
		...
		const geminiSecret = Secret.fromSecretNameV2(
			this,
			"GeminiSecret",
			"gemini-api-key",
		);
		...
		const createCourse = this.lambda(
			"CreateCourse",
			"http/createCourse.ts",
			commonEnvironment,
			lambdaRole,
		);
		const listCourses = this.lambda(
			"ListCourses",
			"http/listCourses.ts",
			commonEnvironment,
			lambdaRole,
		);
		const getDashboard = this.lambda(
			"GetDashboard",
			"http/getDashboard.ts",
			commonEnvironment,
			lambdaRole,
		);
		const createMaterialUpload = this.lambda(
			"CreateMaterialUpload",
			"http/createMaterialUpload.ts",
			commonEnvironment,
			lambdaRole,
		);
		const queueMaterialProcessing = this.lambda(
			"QueueMaterialProcessing",
			"http/queueMaterialProcessing.ts",
			commonEnvironment,
			lambdaRole,
		);
		const listCourseMaterials = this.lambda(
			"ListCourseMaterials",
			"http/listCourseMaterials.ts",
			commonEnvironment,
			lambdaRole,
		);
		const generateStudyPlan = this.lambda(
			"GenerateStudyPlan",
			"http/generateStudyPlan.ts",
			commonEnvironment,
			lambdaRole,
			120,
		);
		const listCourseTasks = this.lambda(
			"ListCourseTasks",
			"http/listCourseTasks.ts",
			commonEnvironment,
			lambdaRole,
		);
		const updateStudyTask = this.lambda(
			"UpdateStudyTask",
			"http/updateStudyTask.ts",
			commonEnvironment,
			lambdaRole,
		);
		const listNotifications = this.lambda(
			"ListNotifications",
			"http/listNotifications.ts",
			commonEnvironment,
			lambdaRole,
		);
		const processMaterial = this.lambda(
			"ProcessMaterial",
			"async/processMaterial.ts",
			commonEnvironment,
			lambdaRole,
			120,
		);
		const sendReminders = this.lambda(
			"SendReminders",
			"async/sendReminders.ts",
			{
				...commonEnvironment,
		...
		const runReminders = this.lambda(
			"RunReminders",
			"http/runReminders.ts",
			{
				...commonEnvironment,
				REMINDER_TOPIC_ARN: reminderTopic.topicArn,
			},
		...
		processMaterial.addEventSource(
			new SqsEventSource(materialQueue, { batchSize: 5 }),
		);
		...
		api.addRoutes({
			...
			methods: [HttpMethod.POST],
			integration: new HttpLambdaIntegration(
				"CreateMaterialUploadIntegration",
				createMaterialUpload,
			),
			authorizer,
		});
		api.addRoutes({
			...
			methods: [HttpMethod.POST],
			integration: new HttpLambdaIntegration(
				"QueueMaterialProcessingIntegration",
				queueMaterialProcessing,
			),
			authorizer,
		});
		api.addRoutes({
			...
			methods: [HttpMethod.GET],
			integration: new HttpLambdaIntegration(
				"ListCourseMaterialsIntegration",
				listCourseMaterials,
			),
			authorizer,
		});
		...
		new CfnOutput(this, "MaterialsBucketName", {
	...
	private configuredReminderEmailAddress(): string {
		...
		if (typeof contextValue === "string" && contextValue.length > 0)
			return contextValue;
		if (process.env.REMINDER_EMAIL_ADDRESS)
			return process.env.REMINDER_EMAIL_ADDRESS;
