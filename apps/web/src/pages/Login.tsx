...
export function Login({ error, isSubmitting = false, onLogin, onRegister, onConfirm }: LoginProps) {
    ...
    return (
      <form className="panel login-panel" onSubmit={handleConfirm}>
        ...
        <p className="login-hint">
          A confirmation code was sent to <strong>{registerDest}</strong>.
        ...
        <button type="button" className="link-btn" onClick={() => { setMode("register"); setLocalError(""); }}>
          ← Back
        </button>
      </form>
    );
  ...
  if (mode === "register") {
    return (
      <form className="panel login-panel" onSubmit={handleRegister}>
        ...
        {displayError ? <p role="alert">{displayError}</p> : null}
        <button type="submit" disabled={isDisabled || !email || password.length < 8}>
          {localSubmitting ? "Creating account..." : "Create account"}
        </button>
        <button type="button" className="link-btn" onClick={() => { setMode("login"); setLocalError(""); }}>
