...
import type { Course, Material, StudyTask, StudyTaskStatus } from "@ai-study-planner/shared";
import {
...
export function CourseDetail({ token }: CourseDetailProps) {
  ...
  const readyMaterial = useMemo(
    () => materials.find((material) => material.status === "ready" && material.summary),
    [materials]
  );
  ...
  useEffect(() => {
    ...
    listCourses(token)
      ...
      .catch(() => {
        if (isCurrent) setError("Could not load course.");
      });
    return () => {
      isCurrent = false;
    };
    ...
    <div className="course-layout">
      ...
      <section className="panel">
        ...
        {readyMaterial?.keyConcepts?.length ? (
          <ul>
            {readyMaterial.keyConcepts.map((concept) => <li key={concept}>{concept}</li>)}
          </ul>
        ) : <p>No concepts yet.</p>}
