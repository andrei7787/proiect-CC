...
export async function queueMaterialProcessing(
	token: string,
	materialId: string,
): Promise<void> {
	await apiRequest<{ queued: boolean; materialId: string }>(
...
export async function listCourseMaterials(
	...
	const response = await apiRequest<ListCourseMaterialsResponse>(
...
export async function listCourseTasks(
	...
	const response = await apiRequest<ListCourseTasksResponse>(
