...
describe("apiRequest", () => {
	...
	it("loads dashboard data from the protected dashboard route", async () => {
		...
		expect(dashboard.summaries[0].summary).toBe(
			"Queues decouple producers from processors.",
		);
		expect(dashboard.notifications[0].message).toBe(
			"Reminder: Review SQS is due today",
		);
		expect(fetchMock).toHaveBeenCalledWith(
			"/dashboard",
			expect.objectContaining({ method: "GET" }),
	...
	it("queues material processing", async () => {
		const fetchMock = vi.fn(
			async () =>
				new Response(JSON.stringify({ queued: true, materialId: "mat-1" }), {
					status: 202,
				}),
		);
		vi.stubGlobal("fetch", fetchMock);
	...
	it("updates a study task status", async () => {
		const fetchMock = vi.fn(
			async () =>
				new Response(JSON.stringify({ taskId: "task-1", status: "done" }), {
					status: 200,
				}),
		);
		vi.stubGlobal("fetch", fetchMock);
	...
	it("triggers reminders for the authenticated user", async () => {
		const fetchMock = vi.fn(
			async () => new Response(JSON.stringify({ sent: 3 }), { status: 200 }),
		);
		vi.stubGlobal("fetch", fetchMock);
